...
function getAccessToken(request, response, callback) {
  ...
  if (expiration <= new Date()) {
    ...
    var refresh_token = getValueFromCookie(
      "node-tutorial-refresh-token",
      ...
      if (error) {
        ...
        var cookies = [
          ...
          "node-tutorial-refresh-token=" +
            ...
            ";Max-Age=4000",
          "node-tutorial-token-expires=" +
            newToken.token.expires_at.getTime() +
            ";Max-Age=4000",
        ];
        response.setHeader("Set-Cookie", cookies);
    ...
    // Return cached token
    var access_token = getValueFromCookie(
      "node-tutorial-token",
      request.headers.cookie
    );
      ...
      client
        ...
        .get((err, res) => {
          if (err) {
            ...
            console.log(
              "getMessages returned " + res.value.length + " messages."
            );
            response.write(
      ...
      client
        ...
        .get((err, res) => {
          if (err) {
            ...
            response.write(
              "<table><tr><th>Subject</th><th>Start</th><th>End</th><th>Attendees</th></tr>"
            );
